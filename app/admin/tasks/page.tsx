...
import {
  detectTaskPlatform,
  generatedTaskThumbnail,
} from '@/lib/task-platform';
...
function normalizeTask(
  ...
  return {
    ...
    social_platform:
      typeof record.social_platform ===
      'string'
        ? record.social_platform
        : null,

    thumbnail_url:
      typeof record.thumbnail_url ===
      'string'
        ? record.thumbnail_url
        : null,

    reward_amount:
      typeof record.reward_amount ===
        'number' ||
      typeof record.reward_amount ===
        'string'
    ...
    total_slots:
      typeof record.total_slots ===
      'number'
        ? record.total_slots
        : null,

    slots_available:
      typeof record.slots_available ===
      'number'
        ? record.slots_available
        : null,
...
export default async function AdminTasksPage({
  ...
  if (missingOptionalColumn) {
    ...
    tasksError =
      fullResponse.error?.message ?? null;
  }
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            <label className="relative md:col-span-2">
              ...
              <input
                ...
                placeholder="Search title, description or task type"
                className="h-11 w-full rounded-xl border border-slate-300 pl-10 pr-3 text-sm outline-none focus:border-green-500"
              />
            </label>
            ...
            <select
              ...
              className="h-11 rounded-xl border border-slate-300 px-3 text-sm capitalize"
            >
              <option value="">
                All statuses
              </option>
            ...
            <div className="flex gap-2">
              <button
                type="submit"
                className="inline-flex h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 text-sm font-bold text-white"
              >
                <FiFilter />
                Filter
              </button>
              ...
              {hasFilters ? (
                <Link
                  ...
                  className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-slate-300"
                >
                  <FiX />
                </Link>
              ) : null}
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4">
                          <Link
                            href={`/tasks/${task.id}`}
                            className="block h-20 w-32 overflow-hidden rounded-xl border border-slate-200 bg-slate-900"
                          >
                            <img
                              src={
                                thumbnail
                        ...
                        <td className="max-w-xs px-5 py-4">
                          <Link
                            href={`/tasks/${task.id}`}
                            className="font-bold text-slate-950 hover:text-green-700"
                          >
                            {task.title ||
                              'Untitled task'}
                          </Link>
